...
This repository contains the Phore Marketplace server daemon which handles the heavy lifting for the Phore Marketplace desktop application. The server combines several technologies: A modified [IPFS](https://ipfs.io) node, which itself combines ideas from Git, BitTorrent, and Kademlia. A lightweight RPC Phore wallet for interacting with the Phore network. And a JSON API which can be used by a user interface to control the node and browse the network. If you are looking for the Phore Marketplace user interface code see [here](https://github.com/projecthelixcoin/openbazaar-desktop).
...
A typical install of PhoreMarketplace contains a bundle of the server daemon and user interface. If this is what you are looking for you can find an installer at https://github.com/projecthelixcoin/openbazaar-desktop/releases. If you are looking to run the server daemon by itself or to contribute to developement see below for instructions.

### Install Pre-built Packages

The easiest way to run the server is to download a pre-built binary. You can find binaries of our latest release for each operating system [here](https://github.com/projecthelixcoin/openbazaar-go/releases/).

### Build from Source

To build from source you will need to have Go installed and properly configured. Detailed instructions for installing Go and openbazaar-go on each operating system can be found in the [docs package](https://github.com/projecthelixcoin/openbazaar-go/tree/master/docs).
...
You can either pull in remote changes as normal or run `go get -u github.com/projecthelixcoin/openbazaar-go`
...
Contributions are definitely welcome! Please read the contributing [guidelines](https://github.com/projecthelixcoin/openbazaar-go/blob/master/CONTRIBUTE.md) before starting.
